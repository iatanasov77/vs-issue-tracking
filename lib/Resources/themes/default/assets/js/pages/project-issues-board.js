...
$( function ()
{
        ...
        if ( addNewBoard ) {
            ...
            function newKanbanbaord()
            {
                ...
                kanbanlisthtml =
                    ...
                    '</div>' +
                    '<div class="my-3">' +
                    '<button class="btn btn-soft-info w-100" data-bs-toggle="modal" data-bs-target="#createTaskModal">Add More</button>' +
                    '</div>' +
                    '</div>';
    ...
    $( '.btnCreateTask' ).on( 'click', function( e )
    {
        ...
        var createTaskUrl =  VsPath( 'vs_issue_tracking_project_issues_kanbanboard_pipeline_create_task', {
        ...
        if ( taskId ) {
            createTaskUrl =  VsPath( 'vs_issue_tracking_project_issues_kanbanboard_pipeline_edit_task', {
                'pipelineId': pipelineId,
                'taskId': taskId
            });
        }
        ...
        $.ajax({
            ...
            success: function( response )
            {
                ...
                if ( parseInt( taskId ) ) {
                    $( '#createTaskModalLabel' ).text( _Translator.trans( 'vs_issue_tracking.form.kanbanboard_task.edit_task' ) );
                    //$( '#btnCreateSubtask' ).text( _Translator.trans( 'vs_issue_tracking.form.kanbanboard_task.edit_task' ) );
                } else {
                    $( '#createTaskModalLabel' ).text( _Translator.trans( 'vs_issue_tracking.form.kanbanboard_task.create_new_task' ) );
                    //$( '#btnCreateSubtask' ).text( _Translator.trans( 'vs_issue_tracking.form.kanbanboard_task.create_new_task' ) );
                }
                ...
                /** Bootstrap 5 Modal Toggle */
                const myModal = new bootstrap.Modal( '#createTaskModal', {
                    keyboard: false
                });
                myModal.show( $( '#createTaskModal' ).get( 0 ) );
            },
            error: function()
            {
                alert( "SYSTEM ERROR!!!" );
    ...
    $( '#modalPipelineTask' ).on( 'click', '#btnCreateIssue', function( e )
    {
        ...
        $.ajax({
            type: "GET",
            url: VsPath( 'vs_issue_tracking_project_issues_kanbanboard_task_create_issue', {
                'pipelineId': pipelineId,
                'parentTaskId': 0
            }),
            success: function( response )
            {
                ...
                $( '#createTaskModalLabel' ).text( _Translator.trans( 'vs_issue_tracking.template.project_issues.create_issue' ) );
                $( '#modalPipelineTask' ).html( response );
    ...
    $( '#modalPipelineTask' ).on( 'submit', '#btnCreateIssueForm', function( e )
    {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function( response )
            {
                var taskId = response.payload.parentTaskId;
                
                var taskUrl = VsPath( 'vs_issue_tracking_project_issues_kanbanboard_pipeline_create_task', {
